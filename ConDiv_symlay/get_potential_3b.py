...
n_node = cb_energy_left.shape[2]

xx = np.arange(1, n_node-1)

for aa in range(20):
    for bl in range(3):
        y1 = ue.clamped_spline_value(cb_energy_left[aa, bl], xx)
        y2 = ue.clamped_spline_value(cb_energy_right[aa, bl], xx)
        y2 += y1[-1] - y2[0]
        p1 = ue.clamped_spline_solve(y1)
        p2 = ue.clamped_spline_solve(y2)
        cb_energy_left[aa, bl]  = p1
        cb_energy_right[aa, bl] = p2
