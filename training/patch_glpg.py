...
WHAT ACTUALLY NEEDS PATCHING. The hybrid config has no side-chain burial, backbone desolvation,
`hbond_coverage` or `hbond_coverage_hydrophobe` node -- it uses the MARTINI environment instead.
So environment.h5 and bb_env.dat do not apply, and of the sidechain arrays only `pair_interaction`
is used by a node whose values change:

    rama_map_pot/rama_pot     <- recomputed from the force field's rama.dat and sheet
    hbond_energy/parameters   <- all twelve, from its hbond.h5
    rotamer/interaction_param <- its pair_interaction

The `placement_*` nodes are copied from `rotamer_center_fixed` and `hydrophobe_placement` at
build time and are NOT patched, so the patch refuses to run if the force field changed those
from the reference the seed was built with: a seed would otherwise silently keep stale ones.

    python3 patch_glpg.py --ff DIR --seed IN.up --out OUT.up [--verify-roundtrip REF_DIR]
...
def rama_library_for(ff_dir):
    ...
    parameters with ff_2.1's glycine row.
    """
...
def check_placement(ff_dir, ref_dir, tol=1e-6):
    """Refuse to patch if the placements the seed was built with are no longer the force field's."""
    with tb.open_file(os.path.join(ff_dir, 'sidechain.h5')) as a, \
            tb.open_file(os.path.join(ref_dir, 'sidechain.h5')) as b:
        for name in ('rotamer_center_fixed', 'hydrophobe_placement'):
            d = float(np.abs(a.get_node('/' + name)[:] - b.get_node('/' + name)[:]).max())
            if d > tol:
                raise ValueError(f'{name} differs from {ref_dir} by {d:.3e}; the seed\'s '
                                 f'placement nodes would be stale, so it must be rebuilt, not patched')
...
def main():
    ...
    ap.add_argument('--reference', default=None,
                    help='force-field directory the seed was built with, for the placement check '
                         '(default: --verify-roundtrip, else $UPSIDE_HOME/parameters/ff_2.1)')
    a = ap.parse_args()
    ref = a.reference or a.verify_roundtrip or os.path.join(os.environ['UPSIDE_HOME'],
                                                            'parameters', 'ff_2.1')
    check_placement(a.ff, ref)
