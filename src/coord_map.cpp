...
struct TwoSigmoidCoord1 : public CoordNode {
    ...
    TwoSigmoidCoord1(hid_t grp, CoordNode& pos_):
        CoordNode(get_dset_size(1, grp, "id")[0],1),
        n_elem(get_dset_size(1, grp, "id")[0]), 
	pos(pos_), 
	params(n_elem),
	...
	n_dim(pos.output.row_width),
        dim1(read_attribute<int>(grp, ".", "dim1"))
    {
        check_size(grp, "c1",    n_elem);
        check_size(grp, "s1",    n_elem);
        check_size(grp, "sign1", n_elem);
        check_size(grp, "c2",    n_elem);
        check_size(grp, "s2",    n_elem);
        check_size(grp, "sign2", n_elem);

        auto& p = params;
        traverse_dset<1,int>  (grp, "id",    [&](size_t i, int   x) {p[i].id    = x;});
        traverse_dset<1,float>(grp, "c1",    [&](size_t i, float x) {p[i].c1    = x;});
        traverse_dset<1,float>(grp, "s1",    [&](size_t i, float x) {p[i].s1    = x;});
        traverse_dset<1,float>(grp, "sign1", [&](size_t i, float x) {p[i].sign1 = x;});
        traverse_dset<1,float>(grp, "c2",    [&](size_t i, float x) {p[i].c2    = x;});
        traverse_dset<1,float>(grp, "s2",    [&](size_t i, float x) {p[i].s2    = x;});
        traverse_dset<1,float>(grp, "sign2", [&](size_t i, float x) {p[i].sign2 = x;});
	assert(dim1<n_dim);
    ...
    virtual void compute_value(ComputeMode mode) override {
        ...
        for(int ne=0; ne<n_elem; ++ne) {
            auto& p   = params[ne];
	    auto x    = pos.output(dim1, p.id);
	    auto sig1 = compact_sigmoid(x-p.c1, p.s1);
	    auto sig2 = compact_sigmoid(x-p.c2, p.s2);
	    if (p.sign1 >= 0 and p.sign2 >= 0 ) {
                output(0, ne) = sig1.x() + sig2.x();
		deriv[ne]     = sig1.y() + sig2.y();
	    }
	    else if (p.sign1 >= 0 and p.sign2 < 0) {
                output(0, ne) = sig1.x() - sig2.x() + 1.f;
		deriv[ne]     = sig1.y() - sig2.y();
            }
	    else if (p.sign1 < 0 and p.sign2 >= 0) {
                output(0, ne) = sig2.x() - sig1.x() + 1.f;
		deriv[ne]     = sig2.y() - sig1.y();
            }
	    else {
                output(0, ne) = - sig2.x() - sig1.x() + 2.f;
		deriv[ne]     = - sig2.y() - sig1.y();
	    }
        }
    }
...
static RegisterNodeType<TwoSigmoidCoord1,1> two_sigmoid1_coord_node("TwoSigmoidCoord1");

struct TwoSigmoidCoord2 : public CoordNode {
    ...
    TwoSigmoidCoord2(hid_t grp, CoordNode& pos_):
        CoordNode(get_dset_size(1, grp, "id")[0],1),
        n_elem(get_dset_size(1, grp, "id")[0]), 
	pos(pos_), 
	params(n_elem),
    ...
    virtual void compute_value(ComputeMode mode) override {
	    ...
	    if (p.sign1 >= 0.f and p.sign2 >= 0.f ) {
                output(0, ne) = sig1.x() * sig2.x();
		deriv[ne]     = sig1.x()*sig2.y() + sig1.y()*sig2.x();
	    }
	    else if (p.sign1 >= 0.f and p.sign2 < 0.f) {
                output(0, ne) = sig1.x()* (1.f-sig2.x());
		deriv[ne]     = sig1.y()*(1.f-sig2.x()) - sig1.x()*sig2.y();
            }
	    else if (p.sign1 < 0.f and p.sign2 >= 0.f) {
                output(0, ne) = sig2.x()*(1.f-sig1.x());
		deriv[ne]     = sig2.y()*(1.f-sig1.x()) - sig2.x()*sig1.y();
            }
	    else {
                output(0, ne) = (1.f-sig1.x())*(1.f-sig2.x());
		deriv[ne]     = - sig1.y()*(1.f-sig2.x()) - (1.f-sig1.x())*sig2.y();
	    }

	    //if (cc%1000 == 0)
	    //    cout << x << " " << 1.f-sig1.x() << " " << sig2.x() << " " << output(0, ne) << endl;
	    //cc++;
        }
    }

    virtual void propagate_deriv() override {
        Timer timer("d_sigmoid2_coord");
        VecArray pos_sens = pos.sens;
        for(int ne=0; ne<n_elem; ++ne) 
            pos_sens(dim1, params[ne].id) += sens(0, ne)*deriv[ne];
    }
...
static RegisterNodeType<TwoSigmoidCoord2,1> two_sigmoid2_coord_node("TwoSigmoidCoord2");
