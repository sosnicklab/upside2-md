...
static inline bool attribute_exists_for_string_attr(hid_t loc_id, const char* obj_name, const char* attr_name) {
...
static std::string read_string_attribute_or_default(hid_t group, const char* attr_name, const std::string& fallback) {
    if(!attribute_exists_for_string_attr(group, ".", attr_name)) return fallback;
...
//Missing: Proper Dihedral from 10.1021/ct700324x
...
struct MartiniPotential : public PotentialNode
{
    ...
    virtual void compute_value(ComputeMode mode) {
        ...
        for(size_t np=0; np<pairs.size(); ++np) {
            ...
            float pair_pot = 0.f;
            if(!eval_pair_force(p1, p2, eps, sig, qi, qj, false, pair_pot, force, 0.f, 0.f, 0.f, 0.f, nullptr, nullptr)) {
                continue;
            }
    ...
    try {
        if(h5_exists(root, "/input/stage_parameters")) {
            ...
            if(enable) {
                ...
                data.stage = read_string_attribute_or_default(
