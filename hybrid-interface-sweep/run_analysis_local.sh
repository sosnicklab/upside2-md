...
if [ "${HYBRID_SWEEP_FORCE_ANALYSIS_INIT:-0}" = "1" ] || [ ! -f "$BASE_DIR/analysis/analysis_manifest.json" ]; then
  "$PYTHON_BIN" "$SCRIPT_DIR/workflow.py" init-analysis --base-dir "$BASE_DIR"
fi

cmd=("$PYTHON_BIN" "$SCRIPT_DIR/workflow.py" run-analysis-local --base-dir "$BASE_DIR")
if [ -n "${HYBRID_SWEEP_ANALYSIS_MAX_TASKS:-}" ]; then
  cmd+=(--max-tasks "$HYBRID_SWEEP_ANALYSIS_MAX_TASKS")
fi
if [ -n "${HYBRID_SWEEP_ANALYSIS_START_TASK:-}" ]; then
  cmd+=(--start-task "$HYBRID_SWEEP_ANALYSIS_START_TASK")
fi
if [ "${HYBRID_SWEEP_OVERWRITE:-0}" = "1" ]; then
  cmd+=(--overwrite)
fi
if [ "${HYBRID_SWEEP_ANALYSIS_NO_ASSEMBLE:-0}" = "1" ]; then
  cmd+=(--no-assemble)
fi
