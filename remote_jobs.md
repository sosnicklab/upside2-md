...
| 53324868 | `nanhunt_glpG-RKRK-79ALA` | **NaN hunt** | PENDING | same, second variant |
| 53325451 | `prep_glpG-RKRK-79HIS` | **POPE/POPG** | PENDING | `run-hybrid-workflow` prep into the equilibrated POPE/POPG bilayer. 12 h wall. |
| 53325452 | `prep_glpG-RKRK-79HIS_S115T` | **POPE/POPG** | PENDING | same |
| 53325453 | `prep_glpG-RKRK-79ALA` | **POPE/POPG** | PENDING | same |
| 53325454 | `prep_glpG-RKRK-79ALA_S115T` | **POPE/POPG** | PENDING | same |
| 53321629 | `np_1AO6_prod` | **NP** | RUNNING | pre-existing self-resubmitting chain |

Paths, all under `/home/yinhanw/project/`:

| campaign | submit script | logs | data |
|---|---|---|---|
| NaN hunt | `glpG_DDM_micelle_REMD/nanhunt.sbatch` | `glpG_DDM_micelle_REMD/nanhunt/logs/nanhunt.<jobid>.out` | `nanhunt/<variant>/` |
| POPE/POPG | `popepopg_glpG/popepopg_prep.sbatch` | `popepopg_glpG/logs/prep.<jobid>.out` | `popepopg_glpG/<variant>/` |

POPE/POPG prep inputs: `popepopg_glpG/inputs/POPEPOPG_equilibrated.pdb` (CHARMM-GUI dry-MARTINI tile
equilibrated locally; packing 3.95 / 4.57 / 4.71 Å against Robertson's 4.21 / 4.60 / 4.71, 166/166
leaflets, P–P 42.75 Å, box 100.132² × 120 Å) and `inputs/2nr9.pdb` (OPM reference, 14.1 Å
half-thickness). Ions are regenerated by the prep at `--salt-molar 0.15` with
`--membrane-thickness-angstrom 42.75` — the headgroup-plane separation `reject_inside_slab` vetoes.

Wall is the caslake QOS ceiling `MaxWall = 1-12:00:00`; each REMD driver guards with `WALL_SEC=129600`
(35-min margin).

### Next actions
1. `grep -E "^===|FATAL|Traceback" popepopg_glpG/logs/prep.*.out` — the prep invocation was submitted but
   **never observed past PENDING**, so it is unverified end to end. Check before trusting any seed.
2. `grep pairdiag glpG_DDM_micelle_REMD/nanhunt/logs/*.out` — a line with a small min distance or a large
   force is the blow-up onset. Then remove the temporary instrumentation from `src/martini_potential.cpp`
   and rebuild both the local and cluster binaries.
3. Only after a verified prep: submit POPE/POPG REMD. Disk is not a constraint — 278 GB estimated for four
   full 12-block chains against 2300 GB free on `/project`.

### Do not forget
- `glpG_DDM_micelle_REMD/glpG-RKRK-79HIS/STOP` exists (from cancelling 53294511) and blocks that variant's
  chain until deleted.
- The cluster binary carries temporary pair diagnostics, inert unless `UPSIDE_MARTINI_PAIR_DIAG` is set
  (verified bitwise-identical physics).
- `run_remd.py` passes a constant `--seed` every chunk, so its NaN rollback re-runs the identical failing
  chunk. The rollback is itself a prohibited guard. Undecided.

```bash
squeue -u yinhanw -o "%.10i %.30j %.9T %.11M %.12l %R"
sacct -u yinhanw -S 2026-08-12 -X -o "JobID%12,JobName%30,State%22,Elapsed,Timelimit,Start%17,End%17,ExitCode"
```
