...
# The membrane comes from a CHARMM-GUI Martini bilayer; its CRYST1 record is the periodic box. Water and
# CHARMM-GUI ions are stripped on read (dry-MARTINI solvent is implicit, ions are rebuilt from SALT_MOLAR).
MEMBRANE_PDB="${MEMBRANE_PDB:-${UPSIDE_HOME}/parameters/dryMARTINI/DOPC.pdb}"
LIPID_NAME="${LIPID_NAME:-DOPC}"
