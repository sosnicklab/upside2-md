...
The workflow now inherits the same thermostat defaults as the standard Upside examples from the engine itself (`tau = 5.0` with the default interval).
Its hybrid stages still use smaller fixed explicit timesteps than the standard examples for stability, so any repo-wide physical time calibration borrowed from those examples should be revalidated before applying it literally to stage `7.0`.
...
The default results are stored in `outputs/martini_test_1rkl_aabb`.  The workflow writes stage checkpoints, logs, and VTF files there.
