...
def read_martini3_atoms(itp_file, molecule_name):
    ...
    for line in lines:
        ...
        if line == molecule_name or line.startswith(molecule_name + " "):
            in_molecule = True
            continue
        
        # Check for atoms section within our molecule
        if in_molecule and line == '[ atoms ]':
            in_atoms = True
            continue
        elif in_molecule and line.startswith('[') and line.endswith(']'):
            in_atoms = False
            continue
...
def read_martini3_angles(itp_file, molecule_name):
    ...
    for line in lines:
        ...
        if in_molecule and line == '[ angles ]':
            in_angles = True
            continue
        elif in_molecule and line.startswith('[') and line.endswith(']'):
            in_angles = False
