...
import csv, glob, math, os, runpy, sys
...
def load_optional_percentd_csv(result_dir, pdb_id):
    ...
    try:
        ...
        print('failed to load percentD csv (%s): %s' % (path, exc))
        return [], []
...
def load_optional_peptide_ids(path, description):
    ...
    if len(rows) < 3:
        print('%s incomplete: %s' % (description, path))
        return None, None, None
...
def load_experimental_reference():
    ...
    matches_map = {}
    for suffix, desc in [
        ('_peps_*.npy', 'experimental peptide list'),
        ('_time_peps_*.npy', 'experimental peptide time grid'),
        ('_d_norm_peps_*.npy', 'experimental peptide uptake curves'),
        ('_d_norm_theor_*.npy', 'experimental theoretical uptake curves'),
    ]:
        matches = glob.glob('%s/%s_%s%s' % (base, primary_pdb_id, HXMS_method, suffix))
        if not matches:
            matches = glob.glob('%s/%s_%s%s%s.npy' % (base, primary_pdb_id, HXMS_method, suffix.replace('*', ''), protein_state))
        if not matches:
    ...
    d_norm_peps_hxms = np.load(matches_map['_d_norm_peps_*.npy'])
    d_norm_theor_hxms = np.load(matches_map['_d_norm_theor_*.npy'])
...
def main():
    ...
    if experimental_reference is None:
        print('HXMS ref files unavailable, skipping comparison')
        sys.exit(0)
    ...
    if len(peps) == 0:
        print('no sim peptide uptake outputs, skipping comparison')
        sys.exit(0)
    ...
    if merge_dfs.empty:
        print('no overlapping peptides between HXMS and sim, skipping comparison')
        sys.exit(0)
    ...
    if len(times) > 0:
        ...
        for pdb_label, time_row, d_norm_row in zip(whole_protein_pdb_ids, times, d_norms):
            log_times = np.asarray(time_row, dtype=float)
            d_norm_values = np.asarray(d_norm_row, dtype=float)
    ...
    for row in merge_dfs.itertuples(index=False):
        exp_time = np.asarray(time_peps_HXMS)
        exp_uptake = np.asarray(row.d_norms_HXMS)
        exp_theory = np.asarray(row.d_theor_HXMS)
        sim_time = np.asarray(row.time_peps)
        sim_uptake = np.asarray(row.d_norms_Up)
        sim_theory = np.asarray(row.d_norms_theor)
    ...
    if not plotted_compare:
        print('no peptide comparison plots generated')
    ...
    cof_HXMS_valid = merge_dfs['COF_HXMS'][np.isfinite(merge_dfs['COF_HXMS'])]
    cof_Up_valid = merge_dfs['COF_Up'][np.isfinite(merge_dfs['COF_Up'])]
    norm_HXMS_res = None
    norm_Up_res = None
    if cof_HXMS_valid.size > 0:
    ...
    colors_HXMS = apply_cmap(-merge_dfs['COF_HXMS'], cmap, norm_HXMS_res) if norm_HXMS_res is not None else ['#cccccc'] * len(merge_dfs)
    colors_Upside = apply_cmap(-merge_dfs['COF_Up'], cmap, norm_Up_res) if norm_Up_res is not None else ['#cccccc'] * len(merge_dfs)
    ...
    peptide_plot(merge_dfs, heights_HXMS, np.asarray(colors_HXMS))
    #peptide_plot(merge_dfs, heights_Upside, colors_Upside_quant)
    peptide_plot(merge_dfs, heights_Upside, np.asarray(colors_Upside))
    ...
    # define colormap and normalization
    cmap = matplotlib.colormaps.get_cmap('bwr').reversed()
    res_cof_hxms_valid = res_avg_int_df_HXMS['res_COF_HXMS'][np.isfinite(res_avg_int_df_HXMS['res_COF_HXMS'])]
    res_cof_up_valid = res_avg_int_df_Up['res_COF_Up'][np.isfinite(res_avg_int_df_Up['res_COF_Up'])]
    diff_color_HXMS = None
    diff_color_Up = None
    if res_cof_hxms_valid.size > 0:
        ...
        diff_color_HXMS = apply_cmap(-res_avg_int_df_HXMS['res_COF_HXMS'], cmap, norm_HXMS)
    if res_cof_up_valid.size > 0:
        ...
        diff_color_Up = apply_cmap(-res_avg_int_df_Up['res_COF_Up'], cmap, norm_Up)
    ...
    if diff_color_HXMS is not None:
        np.save('{}/{}/{}_colors_HXMS.npy'.format(work_dir, fig_dir, primary_pdb_id), diff_color_HXMS)

    np.save('{}/{}/{}_colors_res_Up.npy'.format(work_dir, fig_dir, primary_pdb_id), res_int_df_Up['res_idx'].unique())
    if diff_color_Up is not None:
        np.save('{}/{}/{}_colors_Upside.npy'.format(work_dir, fig_dir, primary_pdb_id), diff_color_Up)
